...
from utils.general import labels_to_class_weights, increment_path, labels_to_image_weights, init_seeds, \
    ...
    check_requirements, print_mutation, set_logging, one_cycle, colorstr, methods, non_max_suppression
from utils.downloads import attempt_download
from utils.loss import ComputeLoss
from utils.plots import plot_labels, plot_evolve
from utils.torch_utils import EarlyStopping, ModelEMA, de_parallel, is_parallel, intersect_dicts, select_device, \
...
def train(hyp,  # path/to/hyp.yaml or hyp dictionary
    ...
    if pretrained:
        ...
        model = Model(cfg, ch=3, nc=nc, anchors=hyp.get('anchors', None)).to(device)  # create
    ...
    random_resize = hyp.get('random_resize', None)
    if random_resize is None:
        multiscale_range = hyp.get('multiscale_range', None)
        if multiscale_range is not None:
            min_size = int(imgsz / 32) - multiscale_range
            max_size = int(imgsz / 32) + multiscale_range
            random_resize = (min_size, max_size)
    sz = None
    model_frist = attempt_load("runs/train/exp108/weights/best.pt", map_location=device)
    model_frist.eval()
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        for i, (imgs, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            ...
            if random_resize is not None:
                if (ni + 1) % 10 == 0:
                    sz = int(32 * random.randint(*random_resize))
            ...
            # 用先验网络处理图片
            outputs = model_frist(imgs)
            out = outputs[0]  # [bs, ,class+5] ->[32,5292,205]
            # Run NMS
            nb, _, height1, width1 = imgs.shape
            ...
            mylist = [int(targets[i, 0].item()) for i in range(targets.shape[0])]   # 存放[0, 0, 2, 2, 3, 3, 3]
            for si, pred in enumerate(out):
                ...
                if pred.shape[0] == 0:
                    ...
                    newimg = mat.to(device, non_blocking=True).float() / 255.0
                    newimgs[si] = newimg
                    continue
                # print("--->", paths[si],pred.shape)
                x_min, y_min, x_max, y_max = min(pred[:, 0]), min(pred[:, 1]), max(pred[:, 2]), max(pred[:, 3])
                bbox_x1, bbox_y1, bbox_x2, bbox_y2 = int(x_min * 0.8), int(y_min * 0.8), int(x_max * 1.2), int(
                    y_max * 1.2)
                ...
                w1 = bbox_x2 - bbox_x1
                h1 = bbox_y2 - bbox_y1
                bbox = [bbox_x1/1920.0, bbox_y1/1920.0]
            ...
            with amp.autocast(enabled=cuda):
                ...
                loss, loss_items = compute_loss(pred, targets)  # loss scaled by batch_size  targets.shape = [200,6]->[bathsize*nl,bathsize,cls,x,y,w,h]
                if RANK != -1:
            ...
            _iter_time = end_iter_time - start_iter_time
            start_iter_time = end_iter_time
            if iter_time is None:
                iter_time = _iter_time
            else:
                iter_time = _iter_time*0.1 + iter_time * 0.9
            # Log
            if RANK in [-1, 0]:
                left_seconds = int((total_nb - ni) * iter_time)
                days = left_seconds // (24*60*60)
                left_seconds = left_seconds - (24*60*60*days)
                hours = left_seconds // (60*60)
                left_seconds = left_seconds - (60*60*hours)
                mins = left_seconds // 60
                seconds = left_seconds - (60*mins)
                left_time = f'{days}days,{hours}:{mins}:{seconds}'
                # if ni == 0:
                #     _file_path = os.path.join(save_dir, 'loss.txt')
                #     _file = open(_file_path, 'a')
                # if ni <= 100:
                #     print(ni, *mloss.tolist(), file=_file)
                #     if ni == 100:
                #         _file.close()
                #         print('\n\nClose File:', _file_path, '\n\n')
                mloss = (mloss * i + loss_items) / (i + 1)  # update mean losses
                mloss_count = mloss.shape[0]
                mem = f'{torch.cuda.memory_reserved() / 1E9 if torch.cuda.is_available() else 0:.3g}G'  # (GB)
                pbar.set_description(('%10s' * 2 + '%10.4g' * (mloss_count+2) + '%18s' + '%10.4gs/it') % (
                    f'{epoch}/{epochs - 1}', mem, *mloss, targets.shape[0], imgs.shape[-1], left_time, iter_time))
...
def parse_opt(known=False):
    ...
    parser.add_argument('--weights', type=str, default='yolox-s.pt', help='initial weights path')
    parser.add_argument('--cfg', type=str, default='yoloxs.yaml', help='model.yaml path')
    parser.add_argument('--data', type=str, default='data/0class.yaml', help='dataset.yaml path')
    parser.add_argument('--hyp', type=str, default='data/hyps/hyp.scratch.yoloxs.yaml', help='hyperparameters path')
