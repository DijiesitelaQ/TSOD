...
class Model(nn.Module):
    def __init__(self, cfg='yolov5s.yaml', ch=3, nc=None, anchors=None):  # model, input channels, number of classes
        ...
        self.loss_fn = self.yaml.get('loss', None)
    ...
    def forward_once(self, x, profile=False, visualize=False):
        ...
        for m in self.model:
            ...
            # if m.type == 'models.common.ImplicitA':
            #     x = m()
            #     y.append(x)
            #     continue
            #
            # if m.type == 'models.common.ControlChannel':
            #     x = m(x,y)
            #     y.append(x)
            #     continue
            # print(m.type)
            ...
            # if m.type == 'models.common.ControlChannel':
            # 可视化代码
            # if m.type != "models.yolox.DetectX":
            #     print(m.type)
            #     draw_feature_map(x, "/home/zeta/LuoQiang/model/Target_Detection/CVPR/YOLOX-Lite/inference/640/62.jpg",m.type,count)
            #     count += 1
...
def parse_model(d, ch):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        if m in [Conv, GhostConv, Bottleneck, GhostBottleneck, SPP, SPPF, DWConv, MixConv2d, Focus, CrossConv,
                 BottleneckCSP, C3, C3TR,C3STR, C3DBB,C3RVGG,CSPT,CSPT2,CSPT3, C3SPP, C3Ghost, RepVGGBlock, CBAM, CoordAtt, CBH, LC_Block, Dense,
                 Shuffle_Block, conv_bn_relu_maxpool, f]:
            c1, c2 = ch[f], args[0]
            if c2 != no:  # if not output
                c2 = make_divisible(c2 * gw, 8)    # 向下取整，取8的倍数

            args = [c1, c2, *args[1:]]
            if m in [BottleneckCSP, C3, C3STR,C3CSTR,C3RVGG,CSPT,CSPT2,CSPT3,C3TR,C3Ghost,C3DBB]:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--cfg', type=str, default='yoloxs.yaml', help='model.yaml')
    # parser.add_argument('--cfg', type=str, default='tph-yoloxs.yaml', help='model.yaml')
