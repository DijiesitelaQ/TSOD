...
def xywhs2xyxy(x, w=640, h=640, padw=0, padh=0, wscale=1, hscale=1):
    # Convert nx4 boxes from [x, y, w, h] normalized to [x1, y1, x2, y2] where xy1=top-left, xy2=bottom-right
    y = x.clone() if isinstance(x, torch.Tensor) else np.copy(x)
    y[:, 0] = w * (x[:, 0] - x[:, 2] / 2) * wscale + padw  # top left x
    y[:, 1] = h * (x[:, 1] - x[:, 3] / 2) * hscale + padh  # top left y
    y[:, 2] = w * (x[:, 0] + x[:, 2] / 2) * wscale + padw  # bottom right x
    y[:, 3] = h * (x[:, 1] + x[:, 3] / 2) * hscale + padh  # bottom right y
    return y
